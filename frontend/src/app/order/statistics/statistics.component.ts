...
interface StatisticsFormValue {
  ...
  grcs: ReadonlyArray<{ grc: Grc; selected: boolean }>;
}

/** Keeps stale statistics while replacements load so charts and key numbers remain visible. */
...
export class StatisticsComponent {
  ...
  readonly stats = withPreviousValue(
    rxResource({
  ...
  readonly perimeterModifiable = computed(() => this.availableGrcs.hasValue() && this.availableGrcs.value().length > 1);
  readonly customerTypeDoughnut = computed(() => {
    return this.stats.hasValue() ? this.createCustomerTypeDoughnutChart(this.stats.value().customerTypeStatistics) : undefined;
  });
  readonly orderStatusDoughnut = computed(() => {
    return this.stats.hasValue() ? this.createOrderStatusDoughnutChart(this.stats.value().orderStatusStatistics) : undefined;
  });
